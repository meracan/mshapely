...
def doc_general():
  ...
  Polygon([(0, 0), (0, 10),(10,10),(10,0),(0,0)]).plot("o",axes[1][0]).resample().plot("o",axes[1][1])\
  .savePlot("doc/img/resample.1.png")
  
  print("")
  print(">>> LineString([(0,0),(30,0)]).dresample(mshapely.DF(np.array([[0,0,1,1.2]]), minDensity=1.0, maxDensity=5.0))")
  fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(8, 3))
  fig.tight_layout()
  LineString([(0,0),(30,0)]).plot("o-",axes[0])\
    .dresample(mshapely.DF(np.array([[0,0,1,1.2]]), minDensity=1.0, maxDensity=5.0)).plot("o-",axes[1])\
    .savePlot("doc/img/dresample.1.png")
  
  print("")
  print(">>> mp = MultiPoint([(100,0)])")
  print(">>> density = np.array([[0,0,100,1.2],[100,0,1,1.2],[200,0,100,1.2]])")
  print(">>> LineString([(0,0),(200,0)]).dresample(mshapely.DF(density,minDensity=2.0, maxDensity=100.0),mp=mp)")
  fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(8, 3))
  fig.tight_layout()
  mp = MultiPoint([(100,0)])
  density = np.array([[0,0,100,1.2],[100,0,1,1.2],[200,0,100,1.2]])
  LineString([(0,0),(200,0)]).plot("o-",axes[0])\
    .dresample(mshapely.DF(density,minDensity=2.0, maxDensity=100.0),mp=mp).plot("o-",axes[1])\
  ...
  print(">>> Polygon([(0, 0), (0, 100),(100,100),(100,0),(0,0)]).dresample(mshapely.DF(density,minDensity=2.0,maxDensity=20.0))")
  print(">>> Polygon([(0, 0), (0, 100),(100,100),(100,0),(0,0)]).dresample(mshapely.DF(density,minDensity=2.0,maxDensity=20.0),mp)")
  ...
  df=mshapely.DF(density,minDensity=2.0,maxDensity=20.0)
  polygon.dresample(df).plot("o-",axes[0][1])
  polygon.dresample(df,mp=mp).plot("o-",axes[1][0])
  ...
  print(">>> polygon.dresample(mshapely.DF(density,minDensity=2.0,maxDensity=50.0),mp)")
  ...
  df=mshapely.DF(density,minDensity=2.0,maxDensity=50.0)
  r = polygon.dresample(df,mp)
  r.plot("o",axes[1])
