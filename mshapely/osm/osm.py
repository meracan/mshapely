...
class OSM(object):
  ...
  def _getosmResample(self,geographicPath=None,projectedPath=None,getDependenciesOnly=False):
    ...
    density=geo.inearest(maxDistance=maxDensity,angle=30.0)
  ...
  def write(*args,**kwargs):
    ...
    # _intput = os.path.splitext(os.path.basename(iinput).replace(".proj",""))[0]
    # n= np.maximum(np.floor(np.log(self.maxDensity/self.minDensity)/np.log(self.growth)-1),1)
    # maxDistance = (self.minDensity*np.power(self.growth,n+1)-self.minDensity)/(self.growth-1)
    # pg_sql = "\"SELECT ST_Union(ST_Buffer(geometry,{0})) FROM '{1}';\"".format(maxDistance,_intput)
    # command = "ogr2ogr -skipfailures -f \"GeoJSON\" {0} -dialect \"SQLITE\" -sql {2} {1}".format(projectedPath,iinput,pg_sql)
    # print(command)
    # subprocess.call(command, shell=True)